...
**juste milieu entre simplicité et performance** : un pipeline honnête (pas de fuite de données),
...
d'un run LightGBM à l'autre. Non retenue comme modèle principal pour rester simple, mais
documentée comme piste testée plutôt qu'ignorée.
...
│   ├── rapport_data_tour_2025.docx        # Version Word source du rapport
...
6. **Modèle** : LightGBM avec hyperparamètres par défaut, `scale_pos_weight` pour le déséquilibre
   et early stopping — pas de réglage manuel arbitraire.
7. **Évaluation** : ROC AUC (probabilités, pas de labels durs — une erreur fréquente qui fausse
   fortement le score), matrice de confusion et rapport de classification à titre indicatif.
